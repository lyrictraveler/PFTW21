...
/*   template: `<tr class="item-container" v-cloak>
    ...
    v-bind:year="post.year"
    v-bind:format="post.format"
    v-bind:description="post.description"
  ></blog-post> 
            
            */
    template: `<div class="item-container" v-cloak>
    <div class="grid">
      <h3>{{ title }}</h3>
      <div class="cell"><img v-bind:src="image" class="photo"></div>
      <div class="cell">{{name}}</div>
      <div class="cell">{{year}}</div>
      <div class="cell">{{format}}}</div>
      <div class="cell__description">{{description}}</div>
    </div>`
