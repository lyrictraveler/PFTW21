...
function draw() { // 1 = draw function
  ...
  // if you had a positive xspeed, you would then use (rightSide <= width) - wh - when I make this change to rightSide because I changed from -1 to 1 above, it seems to break again?  Because it needed to be >= :-))))
  if (rightSide >= width) {
  // announce the count
  noLoop();
  textSize(42);
